...
def insert_plugin_keyword_stats(conn, plugin_slug, keyword_id, stat_date, rank_order, active_installs=0, rating=0, num_ratings=0, downloaded=0):
    ...
    try:
        ...
        ON CONFLICT (plugin_id, keyword_id, stat_date)
        DO UPDATE SET
        ...
        try:
            ...
            # print(f"Raw SQL Query: {raw_query.decode('utf-8')}")  # Print the raw query
           ...
           # print(f"affected_rows: {affected_rows}", flush=True)

            if affected_rows == 0:
                print(f"Warning: No rows were affected for plugin_id: {plugin_id}, keyword_id: {keyword_id}, stat_date: {stat_date}")

            conn.commit()
            print(f"Successfully inserted/updated keyword stats for plugin_id: {plugin_id}, keyword_id: {keyword_id}, stat_date: {stat_date}")

        except Exception as e:
            logging.error(f"Error executing SQL query for plugin {plugin_slug}: {e}")
            conn.rollback()
