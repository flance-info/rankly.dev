...
def insert_plugin_stats(conn, plugin_data):
    ...
    try:
        ...
        now = datetime.now()
        # Use the stat_date from the plugin_data instead of today
        stat_date = plugin_data.get('stat_date')
        
        cursor.execute(sql, (
            plugin_data.get('slug', ''),
            stat_date,  # Using the date from filename
        ...
        conn.commit()
      #  print(f"Successfully inserted stats for plugin: {plugin_data.get('slug')} (Date: {stat_date})")
