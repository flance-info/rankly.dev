            ...
            for plugin in pbar:
                ...
                for tag_slug, tag_name in tqdm(tag_items, 
                    ...
                    try:
                        ...
                        print(f"Raw SQL Query: {raw_query.decode('utf-8')}")
                
                        cur.execute("""
                            INSERT INTO plugin_tags (plugin_slug, tag_slug, created_at, updated_at)
                            VALUES (%s, %s, NOW(), NOW())
                            ON CONFLICT (plugin_slug, tag_slug) DO UPDATE SET updated_at = NOW()
                        """, (plugin_slug, tag_slug))
                        affected_rows = cur.rowcount
                        if affected_rows == 0:
                            print(f"Warning: No rows were affected for plugin_id: {plugin_id}, tag_slug: {tag_slug}")
